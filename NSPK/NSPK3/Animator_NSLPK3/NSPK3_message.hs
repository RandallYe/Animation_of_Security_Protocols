...
module
  NSPK3_message(Dagent(..), equal_dagent, Dnonce(..), equal_dnonce, Dsig(..),
                 equal_dsig, Dskey(..), equal_dskey, Dpkey(..), equal_dpkey,
                 Dmsg(..), equal_dmsg, Chan(..), equal_Chan, atomic, dupl,
                 is_MKs, length, pair2, enum_dagent, enum_dpkey, allPKs, mks,
                 extract_dskey, break_sk, breakm, ma, mn, sn, sp, un_env_C,
                 is_env_C, env, un_sig_C, is_sig_C, sig, mc1, mc2, mem, mkp,
                 allAgents, enum_dnonce, allNonces, allPKsLst, un_fake_C,
                 is_fake_C, fake, un_hear_C, is_hear_C, hear, un_leak_C,
                 is_leak_C, leak, un_recv_C, is_recv_C, recv, un_send_C,
                 is_send_C, send, is_MKp, enc_1, allAgentsLst, allNoncesLst,
                 build_n, extract_dpkey, un_terminate_C, is_terminate_C,
                 terminate, build_n_2)
  where {
...
data Dagent = Alice | Bob | Intruder deriving (Prelude.Read, Prelude.Show);
...
data Dmsg = MAg Dagent | MNon Dnonce | MKp Dpkey | MKs Dskey | MCmp Dmsg Dmsg
  | MEnc Dmsg Dpkey deriving (Prelude.Read, Prelude.Show);
...
break_sk :: [Dmsg] -> Set.Set Dskey -> [Dmsg];
break_sk [] sks = [];
...
is_env_C (Hear_C x4) = False;
is_env_C (Fake_C x5) = False;
...
is_sig_C (Hear_C x4) = False;
is_sig_C (Fake_C x5) = False;
...
enc_1 [] ks = [];
enc_1 (x : xs) ks =
  (if is_MKs x then enc_1 xs ks else map (MEnc x) ks ++ enc_1 xs ks);
...
build_n_2 :: [Dmsg] -> [Dmsg];
build_n_2 ns =
  build_n ns (extract_dpkey ns) (Arith.nat_of_integer (2 :: Integer))
    Arith.one_nat (Arith.nat_of_integer (3 :: Integer));
